...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def _dump_logs(self) -> None:
        ...
        ###############################################################
        CollectLog.log_t['time/total'] = int(time_elapsed)
        CollectLog.log_t['time/timesteps'] = self.num_timesteps
        CollectLog.log_t['training/return_mean'] = safe_mean([ep_info["r"] for ep_info in self.ep_info_buffer])
        CollectLog.log_t['training/length_mean'] = safe_mean([ep_info["l"] for ep_info in self.ep_info_buffer])
        CollectLog.log_t['training/std'] = (self.actor.get_std()).mean().item()

        wandb.log(CollectLog.log_t)
        ###############################################################
