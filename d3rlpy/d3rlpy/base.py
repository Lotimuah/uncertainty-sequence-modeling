...
class LearnableBase:
    ...
    def fitter(
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            # #####################################################################################################
            #
            # with open('{}/thesis/d3rlpy/env_hyperparams.yaml'.format(os.path.expanduser('~')), 'r') as f:
            ...
            # )
            #
            # obs_mean, obs_std = np.mean(dataset.observations, axis=0), np.std(dataset.observations, axis=0)
            #
            # self._model(env, obs_mean, obs_std, total_step, num_eval_episodes=5, max_ep_len=1000)
            #
            # #####################################################################################################
    ...
    #####################################################################################################
    def _evaluate_model(self, env : gym.Env, obs_mean, obs_std, total_step, num_eval_episodes, max_ep_len):

        # create algorithm with saved configuration
        _algo_eval = self.from_json('{}/{}/params.json'.format(os.curdir, self._active_logger.logdir))

        # ready to load
        _algo_eval.load_model('{}/{}/model_{}.pt'.format(os.curdir, self._active_logger.logdir, total_step))

        returns, lengths = [], []

        for _ in range(num_eval_episodes):
            obs = env.reset()
            obs = np.transpose(obs, (0, 3, 1, 2))  # (B, H, W, F) -> (B, F, H, W)

            epi_return, epi_length = 0, 0

            for t in range(max_ep_len):
                act = _algo_eval.predict(obs)
        ...
        print('=' * 68)
        print('return_mean : {}, return_std : {}'.format(np.mean(returns), np.std(returns)))
        print('length_mean : {}, length_std : {}'.format(np.mean(lengths), np.std(lengths)))
        print('=' * 68)

        CollectLog.log['evaluation/return_mean'] = np.mean(returns)
        CollectLog.log['evaluation/return_std'] = np.std(returns)
        CollectLog.log['evaluation/length_mean'] = np.mean(lengths)
        CollectLog.log['evaluation/length_std'] = np.std(lengths)

        wandb.log(CollectLog.log)
    #####################################################################################################
